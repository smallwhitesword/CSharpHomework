...
namespace myOrder
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Customers customer1 = new Customers(1, "张");
                Customers customer2 = new Customers(2, "李");
                Customers customer3 = new Customers(3, "王");
                ...
                Console.WriteLine("");
                Console.WriteLine("通过顾客姓名查询订单:'李'");
                orders = os.GetOrdersByCustomerName("李");
                foreach (Order od in orders)
                    Console.WriteLine(od.ToString());
                Console.WriteLine("");
